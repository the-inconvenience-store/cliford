...
func (g *Generator) generateOperationCmd(sb *StringBuilder, op registry.OperationMeta, reg *registry.Registry, tagPrefix string) {
	...
	// For watch-enabled GET ops the context is created per-iteration inside the closure.
	if op.Timeout != nil && !isWatchEnabledForOp(op, g.watchEnabled) {
	...
	// For watch-enabled GET ops: wrap the request-execute-display block in a for loop.
	// The loop runs once for non-watch mode, repeatedly for watch mode.
	// An immediately-invoked closure captures the iteration body so existing return
	// statements work correctly; the outer loop handles watch-continue vs. stop.
	if isWatchEnabledForOp(op, g.watchEnabled) && g.flagsCfg.Watch.Enabled {
		sb.Line("			for {")
	...
	// Watch-enabled GET ops: always use ctx (defined per-iteration inside closure above).
	// Non-watch ops with timeout: use ctx (defined at top of RunE).
	// Non-watch ops without timeout: use cmd.Context() directly.
	if isWatchEnabledForOp(op, g.watchEnabled) && g.flagsCfg.Watch.Enabled {
	...
	// Close the immediately-invoked closure and the watch for loop.
	if isWatchEnabledForOp(op, g.watchEnabled) && g.flagsCfg.Watch.Enabled {
		sb.Line("			return nil")
		sb.Line("			}()")
